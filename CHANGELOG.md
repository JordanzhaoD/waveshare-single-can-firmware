...
### Removed
- **The v1.18 8.3.6 coordinator family is deleted** (`include/dash_ap_rerequest_activation.h`, 1079 lines; the 60-case `native_ap_rerequest` suite; `docs/AP-REREQUEST-VEHICLE-VALIDATION-PROCEDURE.md`; every wiring point — barrier-A gates, sole-authority branch, auto-rearm taxonomy, vehicle-refusal watch, flag histogram, `appApReRequestTick`). Justification (Route B, Jordan 2026-09-14): the coordinator was a full handshake state machine with a failure taxonomy and auto-re-arm accounting that never went through single-CAN vehicle validation, while the LittleGong firmware (ColorBlack_T-1.3.8) achieves field-reported stability on the same single-CAN wiring with a far simpler machine (disassembly-proven: no retries, no failure classification — a sequence that does not complete silently stops and the human retries). What is deliberately KEPT from our side: the field-validated CRC8-J1850 (3202 real-vehicle frames; LittleGong's table decodes differently and is not blindly copied) and the base injection path. Stale NVS key `apr_on` is actively removed on load. Abort Guard still survives — the JITTER permit cascade runs through it.
- `/defense_config` key `ap_re_request` and the top-level `/status` `apReRequest` diagnostics object are replaced by `jitter` (NVS `jtr_on`, default off; same fail-closed arm semantics — the AP injection gate stays the arm-level precondition).

### Added
- **8.3.6 anti-jerk JITTER procedure** (`DashJitterProcedure`, `include/dash_jitter_procedure.h`, ~430 lines incl. tests-facing docs) — a from-disassembly port of LittleGong's jitter state machine, event-driven on 0x399 `data[0]&0x0F`: Idle → (any nonzero event) Normal → (event 3, AP engage) Arming with a 5000 ms deadline and an immediate one-shot 0x3EE bit46 patched clone (byte5 |= 0x43; no fresh native template ≤5000 ms → skipped fail-closed, cycle continues) → (event 6, FSD lane capture, once per period) Disengaging + a 0x045 cancel burst → (event 1) "cancel seen" → (event 2) re-request burst + deadline refresh; events 8/9/10 and deadline expiry are full resets. The 0x045 pump is carrier-locked like LittleGong's: it only transmits riding behind a native 0x045 frame <47 ms old, ≥3 ms apart, ≤16 frames per burst, first counter = native template counter+1. The cancel lands at the lane-capture entry, ~200 ms ahead of the beta08 jerk window (state 6 + 200-330 ms). The one decoded constant deliberately NOT ported is the "config float > 3.0" event-6 gate — the compared quantity is unidentified and a guessed binding could silently dead-switch the procedure.
- **0x488 steering abort** (same lineage as AbortGuard): while the procedure runs, DAS_trackAngle = ((b0<<8|b1)&0x7FFF − 16384)×0.1° is parsed and gated on `steeringControlType == 2` (beta08 field data — LittleGong parsed the raw value); >45° aborts the current burst, >90° is a full reset. Frame 1160 (0x488) and 962 (0x3C2) are admitted to all three vehicle handler filter tables (11→13 exact ids each) — the recorder and the wheel-DND native-frame visibility ride on the same admission.
- v1.19 base-path pause: while a JITTER procedure runs (Arming/Disengaging) every base device 0x3EE write stops (mux0 AND mux1, via the renamed `jitterInhibit3ee` hook; the ordering pin — before the `!apInjectionGate` early-true — is carried over from the v1.18 sole-authority contract). Idle/Normal leave the base path untouched.
- Minimal defense-page card (开关 + 8 diag cells: 生效/阶段原因/AP周期/取消重挂/帧计数/TX/转向中止/复位), top-level `jitter` diagnostics object on `/status` (chain-rule-disciplined emission), `env:native_jitter` suite (31 cases: full machine paths, constants pins, carrier-lock/3 ms/16-frame pump rules, counter chain, CRC goldens, 0x488 gates, tx-fail silent stop) — added to the tests.yml native matrix (which also closes the v1.18 gap where `native_ap_rerequest` was never listed).
- Wheel DND preflight instrumentation: with 962 admitted, the existing `dashWheelDndCtrl.recordNative3c2` observe branch makes the wheel-DND diag cell a live 0x3C2 visibility indicator (the TX path itself stays gated on the preflight outcome — not in this release).

### Caveats
- The JITTER switch ships **default off**; anti-jerk effectiveness on this firmware is **not yet vehicle-verified**. 「不甩」 remains a single-vehicle small sample; no claim that the steering jerk is fixed. LittleGong's stability was reported on ITS firmware — this is an aligned re-implementation sharing the same wiring.
