...
static CanFrame corruptChecksum(CanFrame f)
{
    f.data[7] ^= 0x55;
    return f;
}
...
static bool buildLateEcho(DashEpasLateEcho &n, unsigned long nowMs, CanFrame &out, bool gatesActive, uint8_t apState, uint8_t hos, const char *gateReason)
{
    DashEpasLateEchoTxToken token = {};
    return n.buildDueFrame(nowMs, out, gatesActive, apState, hos, gateReason, token);
}
...
void test_real_epas_inside_due_window_counts_missed_before_rescheduling()
{
    ...
    TEST_ASSERT_EQUAL_UINT32(1, after.lateWindowMissed);
    TEST_ASSERT_FALSE(buildLateEcho(n, oldDueAt + 1, out, true, 6, 3, nullptr));
...
void test_build_due_frame_after_late_window_expires_clears_pending_and_counts_drop()
{
    ...
    CanFrame out = {};
    TEST_ASSERT_FALSE(buildLateEcho(n, expiredAt, out, true, 6, 3, nullptr));
...
void test_corrupt_checksum_during_warmup_does_not_contribute_to_stability()
{
    DashEpasLateEcho n;
    n.setEnabled(true);
    n.onDasStatus(6, 3, 900, true, nullptr);
    for (uint8_t i = 0; i < 7; ++i)
        n.onEpasFrame(makeEpasFrame(i), 1000 + i * 40, true);
    n.onEpasFrame(corruptChecksum(makeEpasFrame(7)), 1280, true);

    DashEpasLateEchoDiag d = n.diag(1280);
    TEST_ASSERT_FALSE(d.cadenceStable);
    TEST_ASSERT_FALSE(d.pendingEcho);
    TEST_ASSERT_EQUAL_STRING("checksumInvalid", d.blockedReason);
}

void test_corrupt_checksum_while_pending_cancels_and_cannot_build_from_corrupt_source()
{
    DashEpasLateEcho n;
    n.setEnabled(true);
    n.onDasStatus(6, 3, 900, true, nullptr);
    ...
    n.onEpasFrame(corruptChecksum(makeEpasFrame(8)), before.pendingSendAtMs + 1, true);

    CanFrame out = {};
    DashEpasLateEchoDiag after = n.diag(before.pendingSendAtMs + 1);
    TEST_ASSERT_FALSE(after.pendingEcho);
    TEST_ASSERT_EQUAL_STRING("checksumInvalid", after.blockedReason);
    TEST_ASSERT_FALSE(buildLateEcho(n, before.pendingSendAtMs + 1, out, true, 6, 3, nullptr));
}

void test_invalid_non_370_frame_does_not_schedule_from_stale_source()
{
    DashEpasLateEcho n;
    n.setEnabled(true);
    n.onDasStatus(6, 3, 900, true, nullptr);
    ...
    TEST_ASSERT_TRUE(n.buildDueFrame(before.pendingSendAtMs, out, true, 6, 3, nullptr, token));
    n.notifyTxResult(token, true, before.pendingSendAtMs);
    TEST_ASSERT_FALSE(n.diag(before.pendingSendAtMs).pendingEcho);

    CanFrame invalid = makeEpasFrame(8);
    invalid.id = 0x399;
    n.onEpasFrame(invalid, 1320, true);
...
void test_invalid_short_370_frame_does_not_schedule_from_stale_source()
{
    DashEpasLateEcho n;
    n.setEnabled(true);
    n.onDasStatus(6, 3, 900, true, nullptr);
...
void test_build_due_frame_rejects_stale_das_context()
{
    DashEpasLateEcho n;
    n.setEnabled(true);
    n.onDasStatus(6, 3, 900, true, nullptr);
    ...
    CanFrame out = {};
    DashEpasLateEchoTxToken token = {};
    const unsigned long staleAt = 900 + DashEpasLateEcho::kMaxDasStaleMs + 1;
    TEST_ASSERT_FALSE(n.buildDueFrame(staleAt, out, true, 6, 3, nullptr, token));

    DashEpasLateEchoDiag after = n.diag(staleAt);
    TEST_ASSERT_FALSE(after.pendingEcho);
    TEST_ASSERT_EQUAL_STRING("dasStale", after.blockedReason);
    TEST_ASSERT_EQUAL_UINT32(before.gateBlocks + 1, after.gateBlocks);
}

void test_build_due_frame_allows_fresh_das_context()
{
    DashEpasLateEcho n;
    n.setEnabled(true);
    n.onDasStatus(6, 3, 900, true, nullptr);
    for (uint8_t i = 0; i < 8; ++i)
        n.onEpasFrame(makeEpasFrame(i), 1000 + i * 40, true);
    DashEpasLateEchoDiag before = n.diag(1280);
    n.onDasStatus(6, 3, before.pendingSendAtMs - 10, true, nullptr);
...
void test_build_due_frame_requires_final_gate_active_at_send_time()
{
    ...
    TEST_ASSERT_FALSE(n.buildDueFrame(before.pendingSendAtMs, out, false, 5, 4, "finalGateLost", token));
...
void test_build_due_frame_requires_final_ap_active_at_send_time()
{
    DashEpasLateEcho n;
    n.setEnabled(true);
    n.onDasStatus(6, 3, 900, true, nullptr);
    ...
    DashEpasLateEchoTxToken token = {};
    TEST_ASSERT_TRUE(n.due(before.pendingSendAtMs));
    TEST_ASSERT_FALSE(n.buildDueFrame(before.pendingSendAtMs, out, true, 0, 3, nullptr, token));

    DashEpasLateEchoDiag after = n.diag(before.pendingSendAtMs);
    TEST_ASSERT_FALSE(after.pendingEcho);
    TEST_ASSERT_EQUAL_STRING("apInactive", after.blockedReason);
    TEST_ASSERT_EQUAL_UINT8(0, after.lastApState);
    TEST_ASSERT_EQUAL_UINT8(3, after.lastHos);
}

void test_build_due_frame_enters_abort_cooldown_for_final_ap_abort()
{
    DashEpasLateEcho n;
    n.setEnabled(true);
    n.onDasStatus(6, 3, 900, true, nullptr);
    ...
    DashEpasLateEchoTxToken token = {};
    TEST_ASSERT_TRUE(n.due(before.pendingSendAtMs));
    TEST_ASSERT_FALSE(n.buildDueFrame(before.pendingSendAtMs, out, true, 8, 3, nullptr, token));
...
void test_build_due_frame_requires_final_hos_active_at_send_time()
{
    ...
    CanFrame out = {};
    TEST_ASSERT_TRUE(n.due(before.pendingSendAtMs));
    TEST_ASSERT_FALSE(buildLateEcho(n, before.pendingSendAtMs, out, true, 6, 2, nullptr));
...
void test_ap_inactive_update_cancels_existing_pending_echo()
{
    ...
    TEST_ASSERT_FALSE(d.pendingEcho);
    TEST_ASSERT_EQUAL_STRING("apInactive", d.blockedReason);
    TEST_ASSERT_FALSE(buildLateEcho(n, d.pendingSendAtMs, out, true, 6, 3, nullptr));
}
...
void test_gate_loss_during_tx_fail_cooldown_preserves_cooldown()
{
    ...
    n.onDasStatus(6, 3, before.pendingSendAtMs + 100, false, "gateLost");

    DashEpasLateEchoDiag d = n.diag(before.pendingSendAtMs + 100);
    TEST_ASSERT_EQUAL(LateEchoModeState::COOLDOWN, d.mode);
...
void test_inflight_tx_rejects_duplicate_build_without_clearing_token_ownership()
{
    DashEpasLateEcho n;
    n.setEnabled(true);
    n.onDasStatus(6, 3, 900, true, nullptr);
    ...
    CanFrame out = {};
    DashEpasLateEchoTxToken token = {};
    DashEpasLateEchoTxToken duplicate = {};
    TEST_ASSERT_TRUE(n.buildDueFrame(first.pendingSendAtMs, out, true, 6, 3, nullptr, token));
    TEST_ASSERT_FALSE(n.buildDueFrame(first.pendingSendAtMs, out, true, 6, 3, nullptr, duplicate));

    n.notifyTxResult(token, true, first.pendingSendAtMs + 1);
    DashEpasLateEchoDiag after = n.diag(first.pendingSendAtMs + 1);
    TEST_ASSERT_FALSE(after.pendingEcho);
    TEST_ASSERT_EQUAL_UINT32(1, after.sentLateEchoes);
}

void test_delayed_tx_success_for_old_frame_does_not_clear_new_pending_echo()
{
    DashEpasLateEcho n;
    n.setEnabled(true);
    n.onDasStatus(6, 3, 900, true, nullptr);
    ...
    DashEpasLateEchoDiag first = n.diag(1280);
    CanFrame out = {};
    DashEpasLateEchoTxToken oldToken = {};
    TEST_ASSERT_TRUE(n.buildDueFrame(first.pendingSendAtMs, out, true, 6, 3, nullptr, oldToken));
    n.notifyTxResult(oldToken, true, first.pendingSendAtMs);

    n.onEpasFrame(makeEpasFrame(8), 1320, true);
    DashEpasLateEchoDiag newer = n.diag(1320);
    TEST_ASSERT_TRUE(newer.pendingEcho);

    n.notifyTxResult(oldToken, true, 1321);

    DashEpasLateEchoDiag after = n.diag(1321);
    TEST_ASSERT_TRUE(after.pendingEcho);
    TEST_ASSERT_EQUAL_UINT32(newer.pendingSendAtMs, after.pendingSendAtMs);
}

void test_delayed_tx_failure_for_old_frame_does_not_cooldown_new_pending_echo()
{
    DashEpasLateEcho n;
    n.setEnabled(true);
    n.onDasStatus(6, 3, 900, true, nullptr);
    ...
    n.notifyTxResult(oldToken, false, 1321);

    DashEpasLateEchoDiag after = n.diag(1321);
    TEST_ASSERT_TRUE(after.pendingEcho);
    TEST_ASSERT_EQUAL(LateEchoModeState::BURST_ON, after.mode);
    TEST_ASSERT_EQUAL_UINT32(newer.pendingSendAtMs, after.pendingSendAtMs);
    TEST_ASSERT_EQUAL_UINT32(0, after.txFailures);
}
...
void test_build_due_frame_is_single_use_until_tx_result()
{
    ...
    DashEpasLateEchoTxToken secondToken = {};
    TEST_ASSERT_TRUE(n.buildDueFrame(before.pendingSendAtMs, out, true, 6, 3, nullptr, token));
    TEST_ASSERT_FALSE(n.buildDueFrame(before.pendingSendAtMs, out, true, 6, 3, nullptr, secondToken));
    n.notifyTxResult(token, true, before.pendingSendAtMs);
...
void test_build_due_frame_rejects_pending_after_burst_on_expires()
{
    ...
    TEST_ASSERT_FALSE(buildLateEcho(n, 1000, out, true, 6, 3, nullptr));
...
void test_disable_preserves_tx_fail_cooldown_until_expiry()
{
    ...
    TEST_ASSERT_TRUE(n.buildDueFrame(before.pendingSendAtMs, out, true, 6, 3, nullptr, token));
    n.notifyTxResult(token, false, before.pendingSendAtMs);
    TEST_ASSERT_EQUAL(LateEchoModeState::COOLDOWN, n.diag(before.pendingSendAtMs).mode);

    n.setEnabled(false);
    n.setEnabled(true);
    n.onDasStatus(6, 3, before.pendingSendAtMs + 100, true, nullptr);

    DashEpasLateEchoDiag during = n.diag(before.pendingSendAtMs + 100);
    TEST_ASSERT_EQUAL(LateEchoModeState::COOLDOWN, during.mode);
...
int main(int argc, char **argv)
{
    ...
    RUN_TEST(test_corrupt_checksum_while_pending_cancels_and_cannot_build_from_corrupt_source);
    RUN_TEST(test_invalid_non_370_frame_does_not_schedule_from_stale_source);
    RUN_TEST(test_invalid_short_370_frame_does_not_schedule_from_stale_source);
    RUN_TEST(test_build_due_frame_rejects_stale_das_context);
    RUN_TEST(test_build_due_frame_allows_fresh_das_context);
    ...
    RUN_TEST(test_delayed_tx_failure_for_old_frame_does_not_cooldown_new_pending_echo);
