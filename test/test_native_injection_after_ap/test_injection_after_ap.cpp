...
// (dash_ap_first_gate.h removed in v1.18 with the #108 steer-jerk defense
// family; the 12 test_legacy_ap_first_* timing tests and the 2 Minimal
// Inject tests that exercised it were deleted with the module — the 8.3.6
// cancel/re-request coordinator suite in test_native_ap_rerequest now owns
// the activation-jerk behavior.)
...
// (The LegacyApGateHarness + 12 test_legacy_ap_first_* tests were removed in
// v1.18 with the #108 steer-jerk defense family: settle debounce, Instant
// Engage, and Soft Engage no longer exist as admission layers. The kept
// legacyFsdActivationAllowed seam tests below cover the surviving gate
// contract.)
...
// disabled (legacyGateAlwaysAllow stub = apInjectionGate=false short-circuit in
// dashLegacyFsdActivationAllowed), Legacy mux0 must send on the FIRST frame —
// no AP-active requirement. Contrasts the test above (gate ON, stub waits
// until 2000 ms).
...
// (test_legacy_minimal_inject_stops_activation_after_five_and_rearms and
// test_hw3_and_hw4_minimal_inject_stop_only_mux0_activation were removed in
// v1.18 with the #108 steer-jerk defense family: Minimal Inject and the
// DashMinimalInject class no longer exist.)
