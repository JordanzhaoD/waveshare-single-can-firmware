...
> ⚠️ **China 2026.8.3.6 HW3 高风险**：该固件收紧了自动驾驶预检。AP 激活边沿注入 FSD-enable（`0x3EE`）可能触发**方向盘猛甩 + 故障退出**。本固件默认开启 AP-First 门控（注入延迟到 AP 稳定 ~2s 后），**降低但无法消除**该风险（残留 <5%）。建议先用 **Listen-Only 模式**验证。**非 8.3.6 车型**如无需此防护，可在仪表盘「AP 门控」开关关闭以直接注入。详见 `docs/steer-jerk-diagnosis-20260623.md`。
